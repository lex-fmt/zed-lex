...
bin/check                         # full check (fmt, clippy, bats tests)
```

`bin/check` is the single source of truth for quality checks
(delegates to `bin/check-fmt`, `bin/check-lint`, `bin/check-tests`).
