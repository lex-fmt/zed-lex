...
bin/check                         # full check (fmt, clippy, build, manifest, queries)
```

`bin/check` is the CI check entry point and delegates to individual
components (`bin/check-fmt`, `bin/check-lint`, `bin/check-tests`).
Pre-commit is handled by `lefthook.yml` (installed by `release-core init`).
...
suite, compiles the WASM via `bin/build`, bundles it into
`lex-<version>.tar.gz`, creates an annotated tag, and attaches the
