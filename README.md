...
`app-bin/test-all` is the CI check entry point (runs bats files individually
to work around a bats 1.10 tmpdir race). Canonical `bin/check` components
(`bin/check-fmt`, `bin/check-lint`, `bin/check-tests`) are also available.
Pre-commit is handled by `lefthook.yml` (generated by release-sync).
...
```sh
bin/build                     # release WASM (the only thing that ships)
```

`bin/build` runs `cargo build --release --target wasm32-wasip2`.
...
suite, builds `zed-lex-<version>.tar.gz` via `bin/build`,
creates an annotated tag, and attaches the artifact to the GitHub
release.
